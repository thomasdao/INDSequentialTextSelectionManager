...
// Detect word or paragraph surrounds a characterIndex
static NSRange INDRangeForWholeStringAroundIndex(NSString *string, unichar searchCharacter, NSUInteger characterIndex) {
    NSUInteger startIndex = -1;
    NSUInteger endIndex = -1;
    NSUInteger index = characterIndex;
    while (index > 0) {
        unichar previousCharacter = [string characterAtIndex:index-1];
        if (previousCharacter == searchCharacter) {
            startIndex = index;
            break;
        }
        index--;
    ...
    while (index < string.length - 1) {
        unichar nextCharacter = [string characterAtIndex:index+1];
        if (nextCharacter == searchCharacter) {
            endIndex = index;
            break;
        }
        index++;
    ...
    if (event.clickCount == 1) {
        ...
        if (textView && textView.window.firstResponder != textView) {
            ...
            if (event.clickCount == 2) {
                c = [@" " characterAtIndex:0];
            } else {
                c = [@"\n" characterAtIndex:0];
            }
            NSRange range = INDRangeForWholeStringAroundIndex(textView.string, c, index);
